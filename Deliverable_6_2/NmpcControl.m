classdef NmpcControl < handle
    ...
    methods
        ...
        function [u, T_opt, X_opt, U_opt] = get_u(obj, x0, ref)
            ...
            for i = 1:delay
                x_ = x_ + obj.rocket.Ts * obj.rocket.f(x_, u_mem(:, i));
            end
            ...
            if obj.expected_delay > 0
                % Update the memory of control inputs to include the latest input
                % and discard the oldest one, maintaining a fixed-size queue
